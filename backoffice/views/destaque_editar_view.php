    <?php
        if(!empty($_GET["editar"])){
            $id = $_GET["editar"];
            $destaque_unico = destaqueEspecificoID($id);
            if(!empty($_GET["titulo"]) && !empty($_GET["imagem"]) && !empty($_GET["texto"]) && isset($_GET["esta_na_home"])){
                ...
                iduSQL("UPDATE destaques SET titulo ='$titulo', imagem ='$imagem', texto ='$texto', esta_na_home= $esta_na_home, editor='$editor', data_de_edicao='$data_de_edicao' WHERE id='$id'");
        ...
        <?php else: ?>
            ...
            <form class="row">
                ...
                <div class="col">
                    ...
                    <input type="text" name="titulo" size="60" value="<?= $destaque_unico["titulo"]; ?>" required>   
                    <br><br>
                    <label for="imagem" class="pe-2 fs-4">Imagem:</label>
                    <input type="text" name="imagem" size="90" value="<?= $destaque_unico["imagem"]; ?>" required>   
                ...
                <div class="col-12 mt-4">
                    <textarea id="texto" name="texto" cols="160" rows="15" required><?= $destaque_unico["texto"]; ?></textarea>
                    <script>
                ...
                <div class="col-12 mt-5">
                    <select name="esta_na_home" required value ="<?= $destaque_unico["esta_na_home"]; ?>">
                        <option> ---Está na Home---</option>
                        <option value="1" <?=($destaque_unico["esta_na_home"]) ? "selected" : ""; ?>>Sim</option>
                        <option value="0"  <?=(!$destaque_unico["esta_na_home"]) ? "selected" : ""; ?>>Não</option>
                    </select>
                </div>
